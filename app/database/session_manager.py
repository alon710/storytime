...
class SessionManager:
    ...
    async def store_reference_images(
        self, session_id: str, reference_images: list
    ) -> None:
        import pickle
        import base64
        ...
        try:
            async with self.async_session() as db:
                ...
                if session_data:
                    serialized_images = base64.b64encode(
                        pickle.dumps(reference_images)
                    ).decode("utf-8")
                    # Update the dictionary and mark it as modified for SQLAlchemy
                    session_data.collected_fields["reference_images"] = (
                        serialized_images
                    )
            ...
            self.logger.error(
                "Failed to store reference images", session_id=session_id, error=str(e)
            )
    ...
    async def get_reference_images(self, session_id: str) -> list | None:
        ...
        try:
            ...
            self.logger.error(
                "Failed to retrieve reference images",
                session_id=session_id,
                error=str(e),
            )
    ...
    async def approve_seed_image(self, session_id: str, approved: bool) -> None:
        async with self.async_session() as db:
            ...
            if session_data:
                ...
                await db.commit()
                self.logger.info(
                    "Updated seed approval", session_id=session_id, approved=approved
                )

    async def get_seed_approval_status(
        self, session_id: str

...
class StoryTimeAgent:
    def __init__(self):
        ...
        self.seed_tool = SeedTool(settings.seed_model)
        self.narrator_tool = NarratorTool(settings.narrator_model)
        self.illustrator_tool = IllustratorTool(settings.illustrator_model)
        self.storage_tool = StorageTool(settings.storage_backend, settings.session_storage_path)

        self.child_name: Optional[str] = None
        self.child_age: Optional[int] = None
        self.child_gender: Optional[str] = None
        self.challenge_theme: Optional[str] = None
    ...
    async def create_story(
        ...
        self.challenge_theme = challenge_theme
        ...
        try:
            ...
            session_path = await self._save_session(story_book, page_images, seed_images)
            if session_path:
                self.logger.info("Story creation completed successfully", session_path=session_path)
                return session_path
            else:
    ...
    async def _generate_seed_images(self) -> Optional[dict[str, Image.Image]]:
        ...
        child_description = f"{self.child_age}-year-old {self.child_gender} named {self.child_name}"
        child_seed = await self.seed_tool.execute("child", child_description, self.reference_images)
    ...
    async def _generate_story(self):
        ...
        return await self.narrator_tool.execute(
            ...
            self.child_age,
            self.child_gender,
            self.challenge_theme
        )

    async def _generate_illustrations(self, story_book, seed_images) -> Optional[list[Image.Image]]:
        ...
        for page in story_book.pages:
            ...
            if illustration:
                ...
                self.logger.error("Failed to generate illustration for page", page_title=page.title)
                return None

        return page_images

    async def _save_session(self, story_book, page_images, seed_images) -> Optional[str]:
